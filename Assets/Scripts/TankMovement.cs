...
public class TankMovement : NetworkBehaviour,IPointerUpHandler
{
	...
	void HandlePlayerMovement()
	{
        ...
        if (isPressingLeft || Input.GetKey(KeyCode.LeftArrow))
		{
			spin += 1;
            isPressingLeft = false;
            
		}
		if (isPressingRight || Input.GetKey(KeyCode.RightArrow))
		{
			spin -= 1;
		}
		float moveForce = Input.GetAxis("Vertical");

        moveForce = GameObject.Find("Thruster").GetComponent<Scrollbar>().value;
	...
	void AutoMove()
	{
		if (Time.time > autoMoveTimer)
		{
			...
			float thrust = (UnityEngine.Random.Range(-10,10) * 0.1f);
			int spin = UnityEngine.Random.Range(-1,1);
